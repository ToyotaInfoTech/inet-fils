 ...
 *
 * This program is based on "src/inet/networklayer/configurator/ipv4/HostAutoConfigurator.h".
 * 
...
#include "inet/networklayer/configurator/ipv4/HostAutoConfiguratorWifiAp.h"
...
Define_Module(HostAutoConfiguratorWifiAp);

void HostAutoConfiguratorWifiAp::initialize(int stage)
{
...
void HostAutoConfiguratorWifiAp::finish()
{
}

void HostAutoConfiguratorWifiAp::handleMessageWhenUp(cMessage *apMsg)
{
}

void HostAutoConfiguratorWifiAp::setupNetworkLayer()
{
    ...
    while ((ifname = interfaceTokenizer.nextToken()) != nullptr) {
        ...
        case 'c': //cellular
            ...
            myAddress = Ipv4Address( wlanAddr.getInt() + 10 * 256 + 2 + uint32(host->getIndex()));
            netmask = Ipv4Address(par("netmask").stringValue());
            ie->setBroadcast(true);
            break;
        case 'w': //wlan0
            myAddress = Ipv4Address( wlanAddr.getInt() + 100 + uint32(host->getIndex()));
            netmask = Ipv4Address("255.255.0.0");

            // Add a default route to routing table of the host
            if (routingTable) {
                Ipv4Route *route = new Ipv4Route();
                route->setDestination(Ipv4Address("0.0.0.0"));
                route->setNetmask(Ipv4Address("0.0.0.0"));
                route->setInterface(ie);
                route->setMetric(100);
                routingTable->addRoute(route);
            }
            else {
                EV_WARN << "No routing table found" << endl;
            }
